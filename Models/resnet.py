...
class ResNet(nn.Module):
  ...
  def __init__(self, block, layers, num_classes=1000, modified_num_classes = 20, modified=True, zero_init_residual=False,
      ...
      if modified:
          self.modified_fc = nn.Linear(512 * block.expansion, modified_num_classes)
      else:
          self.modified_fc = None
  ...
  def forward_classifier(self, x):
      x = self.avgpool(self.layer4_out)
      x = torch.flatten(x, 1)
      if self.modified_fc != None:
          return self.modified_fc(x)
      return self.fc(x)
